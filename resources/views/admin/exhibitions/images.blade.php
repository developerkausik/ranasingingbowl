    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Exhibition Image Management</h4>
                        <p class="card-description"> Add/ Update Image of {{ $exhibition->title }}</p>
                        ...
                        <form class="forms-sample" id="newFrmId" name="newFrmName" method="POST" action="#"
                            ...
                            <input type="hidden" name="exhibition_id" id="exhibition_id"
                                value="@if(isset($exhibition->id) && $exhibition->id > 0){{ $exhibition->id }}@endif">
                            <input type="hidden" name="id" id="id"
                                value="@if(isset($image->id) && $image->id > 0){{ $image->id }}@endif">
                            <div class="form-group">
                                <label for="exampleInputName1">Image Title</label>
                                <input type="text" class="form-control" id="title" name="title"
                                    placeholder="Image Title"
                                    value="@if(isset($image->title) && $image->title != ''){{ $image->title }}@else{{ old('title') }}@endif">
                            </div>
                            <div class="form-group">
                                <label>Upload Image</label>
                                @if (isset($image->image) && $image->image != '')
                                    <div class="input-group col-xs-12" style="margin-bottom: 20px;">
                                        <img src="{{ URL::asset($image->image) }}" alt="Exhibition Image" style="max-width: 100%;">
                                    </div>
                                @endif
                            ...
                            <button type="button" class="btn btn-danger mr-2"
                                onclick="window.location.href='{{ route('adminExhibitions') }}'"><i
                                    class="ti-arrow-left btn-icon-prepend"></i> Back to Exhibition</button>
            ...
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-9">
                                <h4 class="card-title">List of Images for {{ $exhibition->title }}</h4>
                            </div>
                            {{-- <div class="col-sm-3">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if (isset($val->image) && $val->image != '')
                                                        <img src="{{ URL::asset($val->image) }}" alt="{{ $val->title }}" style="max-width: 100px;">
                                                    @else
                                                        No Image
                                                    @endif
                                                </td>
                                                <td><a href="{{ route('adminExhibitionImagesUpdate', [Crypt::encryptString($exhibition->id), Crypt::encryptString($edit_id)]) }}"
        ...
        $(document).ready(function() {
            $('#dataTable').on('click', '.delete', function() {
                ...
                if (msg) {
                    ...
                    $('#proccedtologin').show();
                    $.ajax({
                        url: "{{ route('adminExhibitionImagesDelete') }}",
                        data: {
                            id: id
                        },
                        type: 'post',
            ...
            $('#newFrmId').submit(function() {
                ...
                $('#proccedtologin').show();
                $.ajax({
                    url: "{{ route('adminExhibitionImagesDataSubmit') }}",
                    data: formData,
                    type: 'post',
                    cache: false,
                    processData: false,
                    ...
                    success: function(data) {
                        ...
                        setTimeout(function() {
                            var url =
                                "{{ route('adminExhibitionImages', ['id' => 'ID_PLACEHOLDER']) }}";
