    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Exhibition Management</h4>
                        <p class="card-description"> Add/ Update Exhibition Content </p>
                        ...
                        <form class="forms-sample" id="newFrmId" name="newFrmName" method="POST" action="#"
                            ...
                            <div class="form-group">
                                <label for="exampleInputName1">Exhibition Date</label>
                                <input type="date" class="form-control date-control" id="exhibition_date" name="exhibition_date"
                                    placeholder="Exhibition Date"
                                    value="@if(isset($result->exhibition_date) && $result->exhibition_date != ''){{ $result->exhibition_date }}@else{{ old('exhibition_date') }}@endif">
                            </div>
                            <div class="form-group">
                                <label>Status</label>
                                <select class="form-select" id="status" name="status">
                                    <option value="1" @if (isset($result->status) && $result->status == 1) selected @endif>Active
                                    </option>
                                    <option value="0" @if (isset($result->status) && $result->status == 0) selected @endif>Inactive
                                    </option>
                                </select>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#newFrmId').submit(function() {
                ...
                $('#proccedtologin').show();
                $.ajax({
                    url: "{{ route('adminExhibitionsDataSubmit') }}",
                    data: formData,
                    type: 'post',
                    cache: false,
                    processData: false,
