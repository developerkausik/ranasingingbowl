            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-description"> Add/ Update Category Content </p>
                        ...
                        <form class="forms-sample" id="newFrmId" name="newFrmName" method="POST" action="#"
                            ...
                            <div class="form-group">
                                <label for="exampleInputName1">Manufactured</label>
                                <input type="text" class="form-control" id="manufactured" name="manufactured" placeholder="Title"
                                    value="@if(isset($result->manufactured) && $result->manufactured != ''){{ $result->manufactured }}@else{{ old('manufactured') }}@endif">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="sort_order" name="sort_order" placeholder="Sort Order"
                                    value="@if(isset($result->sort_order) && $result->sort_order != ''){{ $result->sort_order }}@else{{ old('sort_order') }}@endif">
                            </div>
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="meta_title" name="meta_title" placeholder="Meta Title"
                                    value="@if(isset($result->meta_title) && $result->meta_title != ''){{ $result->meta_title }}@else{{ old('meta_title') }}@endif">
                            </div>
                            <div class="form-group">
                                <label for="exampleInputName1">Meta Keywords</label>
                                <input type="text" class="form-control" id="meta_keyword" name="meta_keyword" placeholder="Meta Keywords"
                                    value="@if(isset($result->meta_keyword) && $result->meta_keyword != ''){{ $result->meta_keyword }}@else{{ old('meta_keyword') }}@endif">
                            </div>
                            <div class="form-group">
                                <label for="exampleInputName1">Meta Description</label>
                                <textarea class="form-control" id="meta_description" name="meta_description" placeholder="Meta Description">@if (isset($result->meta_description) && $result->meta_description != ''){{ $result->meta_description }}@else{{ old('meta_description') }}@endif</textarea>
                            </div>
                            <button type="button" class="btn btn-danger mr-2"
                                onclick="window.location.href='{{ route('adminProductCategory') }}'"><i
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#newFrmId').submit(function() {
                ...
                $('#proccedtologin').show();
                $.ajax({
                    url: "{{ route('adminProductCategoryDataSubmit') }}",
                    data: formData,
                    type: 'post',
                    cache: false,
                    processData: false,
