...
@section('content')
            ...
            <div class="row">
                            ...
                            <div class="product-view-slider">
                                <div class="prdct-slider-outr">
                                        ...
                                        @if (isset($product->images) && count($product->images) > 0)
                                            @foreach ($product->images as $key => $image)
                                                <div class="item">
                                                    ...
                                                    <img src="{{ URL::asset($image->image) }}" class="zoom-img"
                                                        data-zoom-image="{{ URL::asset($image->image) }}" />
                                                    <div class="slide-text text-center">
                                                        <h5>Product Code: #{{ $image->product_code }}</h5>
                                                    </div>
                                                </div>
                                            @endforeach
                                        @endif
                                    ...
                                    <div id="sync2" class="owl-carousel owl-theme">
                                        @if (isset($product->images) && count($product->images) > 0)
                                            @foreach ($product->images as $key => $image)
                                                <div class="item"><img src="{{ URL::asset($image->image) }}"
                                                        alt="{{ $image->title }}" /></div>
                                            @endforeach
                                        @endif
                                    </div>
                ...
                <div class="col-lg-7 prdct-details-content-outr company-profile-text-col" data-aos="fade-left">
                    <div class="welcome-heading">
                        <h2 data-aos="fade-up" class="aos-init aos-animate">{{ $product->title }}</h2>
                    </div>
                    @if (isset($product->sounds) && count($product->sounds) > 0)
                        <div class="d-flex align-items-center ">
                            <h5>Check Sound :</h5>
                            <div class="sample-audio--wrap d-flex ">
                                @foreach ($product->sounds as $key => $sound)
                                    @if ($sound->variant_id == '')
    ...
    <div class="modal fade" id="exampleModalquery2" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <section class="variants-sec-outr product-outr">
            <div class="container" data-aos="zoom-in">
                ...
                <div class="text-center mt-4 contact-form-box-col">
                    <button type="button" onclick="location.href='{{ route('productVariantAll', [$category->slug, createSlug($product->title)]) }}'" class="btn shop-link">View All Variants</button>
                </div>
            </div>
        </section>
    @endif
...
@section('script')
        ...
        $(document).ready(function() {
            $('#productEnquiryFrm').on('submit', function(e) {
                ...
                grecaptcha.ready(function() {
                    grecaptcha.execute('{{ config('services.recaptcha.site_key') }}', { action: 'product_enquiry' }).then(function(token) {

                        // Append or update the token in the form
                        if ($('#productEnquiryFrm input[name="g-recaptcha-response"]').length === 0) {
                            $('#productEnquiryFrm').prepend('<input type="hidden" name="g-recaptcha-response" value="' + token + '">');
                        } else {
                            $('#productEnquiryFrm input[name="g-recaptcha-response"]').val(token);
                        }

                        var formData = $('#productEnquiryFrm').serialize();
                        $('#proccedtologin').show();
    ...
    <link data-stencil-stylesheet href="{{ URL::asset('css/audio.css') }}" rel="stylesheet">
